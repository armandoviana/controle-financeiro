...
// RECORRENTES - Definir primeiro para garantir disponibilidade
...
async function carregarPrevisoes() {
    try {
        ...
        if (!prevs || prevs.length === 0) {
            container.innerHTML = '<p style="text-align:center;color:rgba(255,255,255,0.6);padding:2rem;">Sem dados suficientes. Adicione gastos nos últimos 3 meses.</p>';
            return;
        }
            ...
            return `
                <div class="previsao-card">
                    <div class="previsao-header">
                        <h3>${p.categoria}</h3>
                        <span class="previsao-icon">📊</span>
                    </div>
                    <div class="previsao-valores">
                        <div class="previsao-item">
                            <span class="previsao-label">Previsão (média ${p.base_meses} meses)</span>
                            <span class="previsao-valor">${formatarMoeda(p.previsao)}</span>
                        </div>
                    </div>
                </div>
            `;
